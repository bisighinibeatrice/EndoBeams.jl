...
struct Encastre 
    blocked_dofs::Union{Int, UnitRange{Int}, Vector{Int}}  # Degrees of freedom (DOFs) that are fully fixed
end
 
# Structure representing imposed displacement boundary conditions
struct ImposedDisplacement{TD}
    displaced_dofs::Union{Int, UnitRange{Int}, Vector{Int}}  # DOFs where displacements are imposed
...
struct BoundaryConditions{TD} 
    fixed_dofs::Union{Int, UnitRange{Int}, Vector{Int}}  # DOFs that are fixed
    free_dofs::Union{Int, UnitRange{Int}, Vector{Int}}   # DOFs that are free to move
    displaced_indices::Union{Int, UnitRange{Int}, Vector{Int}}  # Indices of imposed displacement DOFs among the fixed DOFs
...
function BoundaryConditions(bcs::Encastre, ndofs) 
    ...
    return BoundaryConditions{Nothing}(fixed_dofs, free_dofs, displaced_indices, imposed_displacements, nothing)
end

# Constructor for BoundaryConditions with imposed displacement conditions
function BoundaryConditions(bcs::ImposedDisplacement, ndofs) 
    ...
    return BoundaryConditions{typeof(bcs.imposed_displacements_function)}(fixed_dofs, free_dofs, displaced_indices, imposed_displacements, bcs.imposed_displacements_function)
end 

# Constructor for BoundaryConditions combining encastre and imposed displacement conditions
function BoundaryConditions(bcs1::Encastre, bcs2::ImposedDisplacement, ndofs) 
    ...
    return BoundaryConditions{typeof(bcs2.imposed_displacements_function)}(fixed_dofs, free_dofs, displaced_indices, imposed_displacements, bcs2.imposed_displacements_function)
end
