...
Base.@propagate_inbounds function compute(u₁, Θ₁, u₂, Θ₂, u̇₁, u̇₂, ẇ₁, ẇ₂, ü₁, ü₂, ẅ₁, ẅ₂, exact=false, dynamics=true)
    ...
    if !dynamics
        ...
        for iG in 1:comp.nᴳ
            ...
            C̃ₖ¹¹ += ωᴳ * (Aᵨ*H₁¹'*C₁¹ + H₂¹'*Īᵨ*C₂¹ + H₂¹'* SẆ₀Īᵨ *H₂¹)
            C̃ₖ¹² += ωᴳ * (Aᵨ*H₁¹'*C₁² + H₂¹'*Īᵨ*C₂² + H₂¹'* SẆ₀Īᵨ *H₂²)
            C̃ₖ¹³ += ωᴳ * (Aᵨ*H₁¹'*C₁³ + H₂¹'*Īᵨ*C₂³ + H₂¹'* SẆ₀Īᵨ *H₂³)
            C̃ₖ¹⁴ += ωᴳ * (Aᵨ*H₁¹'*C₁⁴ + H₂¹'*Īᵨ*C₂⁴ + H₂¹'* SẆ₀Īᵨ *H₂⁴)
            C̃ₖ²¹ += ωᴳ * (Aᵨ*H₁²'*C₁¹ + H₂²'*Īᵨ*C₂¹ + H₂²'* SẆ₀Īᵨ *H₂¹)
        ...
        # Check that Tₖ = C̃ₖ ḋ + M d̈
        @test Tₖ¹ ≈ (C̃ₖ¹¹*u̇₁ + C̃ₖ¹²*ẇ₁ + C̃ₖ¹³*u̇₂ + C̃ₖ¹⁴*ẇ₂) + (M¹¹ *ü₁ + M¹² *ẅ₁ + M¹³ *ü₂ + M¹⁴*ẅ₂) atol=1e-6
        @test Tₖ² ≈ (C̃ₖ²¹*u̇₁ + C̃ₖ²²*ẇ₁ + C̃ₖ²³*u̇₂ + C̃ₖ²⁴*ẇ₂) + (M¹²'*ü₁ + M²² *ẅ₁ + M²³ *ü₂ + M²⁴*ẅ₂) atol=1e-6
        @test Tₖ³ ≈ (C̃ₖ³¹*u̇₁ + C̃ₖ³²*ẇ₁ + C̃ₖ³³*u̇₂ + C̃ₖ³⁴*ẇ₂) + (M¹³'*ü₁ + M²³'*ẅ₁ + M³³ *ü₂ + M³⁴*ẅ₂) atol=1e-6
        @test Tₖ⁴ ≈ (C̃ₖ⁴¹*u̇₁ + C̃ₖ⁴²*ẇ₁ + C̃ₖ⁴³*u̇₂ + C̃ₖ⁴⁴*ẇ₂) + (M¹⁴'*ü₁ + M²⁴'*ẅ₁ + M³⁴'*ü₂ + M⁴⁴*ẅ₂) atol=1e-6
        ...
        # Check that kinetic_energy = 0.5 ḋ' M ḋ
        @test kinetic_energy ≈ (u̇₁'* (M¹¹ *u̇₁ + M¹² *ẇ₁ + M¹³ *u̇₂ + M¹⁴*ẇ₂) + 
                                ẇ₁'* (M¹²'*u̇₁ + M²² *ẇ₁ + M²³ *u̇₂ + M²⁴*ẇ₂) + 
                                u̇₂'* (M¹³'*u̇₁ + M²³'*ẇ₁ + M³³ *u̇₂ + M³⁴*ẇ₂) + 
                                ẇ₂'* (M¹⁴'*u̇₁ + M²⁴'*ẇ₁ + M³⁴'*u̇₂ + M⁴⁴*ẇ₂))/2  atol=1e-6
...
function finitediff_statics(exact=true)
    ...
    _, ftest, Ktest= compute(u₁, Θ₁, u₂, Θ₂, u̇₁, u̇₂, ẇ₁, ẇ₂, ü₁, ü₂, ẅ₁, ẅ₂, exact, false)
    ...
    for i in 1:12
        ...
        e⁺, f⁺, _ = compute(u₁⁺, Θ₁⁺, u₂⁺, Θ₂⁺, u̇₁, u̇₂, ẇ₁, ẇ₂, ü₁, ü₂, ẅ₁, ẅ₂, true, false)
        e⁻, f⁻, _ = compute(u₁⁻, Θ₁⁻, u₂⁻, Θ₂⁻, u̇₁, u̇₂, ẇ₁, ẇ₂, ü₁, ü₂, ẅ₁, ẅ₂, true, false)
        F[i] = (e⁺-e⁻)/(2*ε)
        H[:, i] .= (f⁺-f⁻)/(2*ε)
    end
...
@test finitediff_statics(true) ≈ [0, 0] atol=1e-6



compute(u₁, Θ₁, u₂, Θ₂, u̇₁, u̇₂, ẇ₁, ẇ₂, ü₁, ü₂, ẅ₁, ẅ₂, true, true)